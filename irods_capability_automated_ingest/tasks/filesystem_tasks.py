...
def filesystem_sync_path(self, meta):
    ...
    try:
        ...
        if not utils.delete_mode_is_compatible_with_operation(delete_mode, operation):
            raise RuntimeError(
                f"delete_mode [{delete_mode}] and operation [{operation}] are incompatible."
            )
        ...
        for obj in itr:
            ...
            if not obj.is_symlink() and not bool(mode & stat.S_IRGRP):
                ...
                # This is going to cause a KeyError below. If we want to just ignore these, don't put it in the chunk.
                # chunk[full_path] = {}
...
def filesystem_sync_entry(self, meta_input, datafunc, metafunc):
    ...
    try:
        ...
        # path == the current (source) path being synced
        # target2 == the current collection
